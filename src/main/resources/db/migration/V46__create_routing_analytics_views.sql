...
FROM (
    SELECT
        ROUND(origin_latitude::NUMERIC, 3)::DOUBLE PRECISION AS lat,
        ROUND(origin_longitude::NUMERIC, 3)::DOUBLE PRECISION AS lon,
        'origin'::TEXT AS point_type
    FROM trip_plans
    UNION ALL
    SELECT
        ROUND(destination_latitude::NUMERIC, 3)::DOUBLE PRECISION AS lat,
        ROUND(destination_longitude::NUMERIC, 3)::DOUBLE PRECISION AS lon,
...
SELECT
    ROUND(origin_latitude::NUMERIC, 2)::DOUBLE PRECISION AS from_lat,
    ROUND(origin_longitude::NUMERIC, 2)::DOUBLE PRECISION AS from_lon,
    ROUND(destination_latitude::NUMERIC, 2)::DOUBLE PRECISION AS to_lat,
    ROUND(destination_longitude::NUMERIC, 2)::DOUBLE PRECISION AS to_lon,
...
FROM trip_plans tp
GROUP BY
    ROUND(origin_latitude::NUMERIC, 2),
    ROUND(origin_longitude::NUMERIC, 2),
    ROUND(destination_latitude::NUMERIC, 2),
    ROUND(destination_longitude::NUMERIC, 2)
HAVING COUNT(*) >= 2;
