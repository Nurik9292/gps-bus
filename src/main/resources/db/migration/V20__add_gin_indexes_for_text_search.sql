...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bus_stops_stop_name_gin_trgm
    ON bus_stops USING gin (stop_name gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bus_stops_name_en_gin_trgm
    ON bus_stops USING gin (name_en gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bus_stops_name_tm_gin_trgm
    ON bus_stops USING gin (name_tm gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bus_routes_route_number_gin_trgm
    ON bus_routes USING gin (route_number gin_trgm_ops);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_bus_routes_route_name_gin_trgm
