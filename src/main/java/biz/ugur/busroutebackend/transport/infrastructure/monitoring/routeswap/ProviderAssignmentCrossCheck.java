...
public class ProviderAssignmentCrossCheck {
    ...
    @Scheduled(cron = "${business.route-swap-detector.provider-check-cron:0 30 6,15 * * *}",
            zone = "Asia/Ashgabat")
    ...
    private Mono<Void> sendDigest(List<String> freshLines) {
        if (freshLines.isEmpty() || quietHours.active()) {
            return Mono.empty();
        }
        String subject = "[ROUTE_SWAP] расхождения с реестром провайдера: " + freshLines.size();
