...
@Service
@Slf4j
public class GetRouteAssignmentsByVehicleUseCase extends BaseUseCase<Mono<String>, List<RouteAssignmentData>> {

    private final RouteAssignmentRepository assignmentRepository;
    private final RouteAssignmentDataMapper dataMapper;

    public GetRouteAssignmentsByVehicleUseCase(
    ...
    @Override
    protected Mono<List<RouteAssignmentData>> process(Mono<String> request) {
    ...
    private Mono<List<RouteAssignmentData>> processInternal(String vehicleIdStr) {
        VehicleId vehicleId = VehicleId.of(vehicleIdStr);

        return assignmentRepository.findActiveByVehicleId(vehicleId)
                .flatMap(dataMapper::toRouteAssignmentData)
                .collectList()
                .doOnSuccess(data -> log.debug("Retrieved {} route assignments for vehicle: {}",
                        data.size(), vehicleIdStr))
                .doOnError(error -> log.error("Failed to retrieve route assignments for vehicle: {}",
                        vehicleIdStr, error));
    }
}
