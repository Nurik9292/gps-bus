...
@Service
@Slf4j
public class RejectAdPlacementUseCase
        extends BaseUseCase<RejectAdPlacementUseCase.Request, AdPlacementResponse> {

    public record Request(String placementId, String adminId, RejectAdPlacementCommand command) {}

    private final AdPlacementRepository placementRepository;
    private final AdPlacementResponseMapper responseMapper;

    public RejectAdPlacementUseCase(AdPlacementRepository placementRepository,
    ...
    protected Mono<AdPlacementResponse> process(Request req) {
        return placementRepository.findById(PlacementId.of(req.placementId()))
                ...
                .map(p -> p.reject(req.adminId(), req.command().reason()))
                .flatMap(placementRepository::save)
                .transform(this::persistAndPublish)
                .flatMap(responseMapper::toResponse)
                .doOnSuccess(r -> log.info("AdPlacement rejected: id={} by adminId={}",
                        r.id(), req.adminId()));
    }
