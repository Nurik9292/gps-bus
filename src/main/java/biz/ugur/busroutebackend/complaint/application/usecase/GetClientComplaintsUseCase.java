...
@Service
@Slf4j
public class GetClientComplaintsUseCase extends BaseUseCase<Mono<GetClientComplaintsInput>, ComplaintList> {

    private final ComplaintRepository complaintRepository;

    public GetClientComplaintsUseCase(ComplaintRepository complaintRepository,
    ...
    @Override
    protected Mono<ComplaintList> process(Mono<GetClientComplaintsInput> request) {
        return request.flatMap(input -> {
            ...
            return Mono.zip(
                    complaintRepository.findByClientId(input.getClientId(), pageable)
                            .map(ComplaintResult::fromDomain)
                            .collectList(),
                    complaintRepository.countByClientId(input.getClientId())
