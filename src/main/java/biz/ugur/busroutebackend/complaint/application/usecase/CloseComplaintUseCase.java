...
@Service
@Slf4j
public class CloseComplaintUseCase extends BaseUseCase<Mono<UpdateComplaintStatusInput>, ComplaintResult> {

    private final ComplaintRepository complaintRepository;

    public CloseComplaintUseCase(ComplaintRepository complaintRepository,
    ...
    protected Mono<ComplaintResult> process(Mono<UpdateComplaintStatusInput> request) {
        return request.flatMap(input ->
                complaintRepository.findById(ComplaintId.of(input.complaintId()))
                        ...
                        .map(complaint -> complaint.close(input.adminComment()))
