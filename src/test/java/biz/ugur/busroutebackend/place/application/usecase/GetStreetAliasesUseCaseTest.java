...
class GetStreetAliasesUseCaseTest {

    @InjectMocks
    private GetStreetAliasesUseCase useCase;
    ...
    void returnsAliasList() {
        StreetAlias alias = StreetAlias.create("street-1", "MainRoad", "en");

        when(correlationService.getCurrentCorrelationId()).thenReturn(Mono.just(CorrelationId.generate()));
        when(correlationService.executeWithCorrelation(any(Mono.class), anyString()))
                .thenAnswer(inv -> inv.getArgument(0));
        when(streetAliasRepository.findByStreetId("street-1")).thenReturn(Flux.just(alias));

        StepVerifier.create(useCase.execute(Mono.just("street-1")))
