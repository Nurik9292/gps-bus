...
class FleetPresenceClassificationTest {
    ...
    private final LocalDateTime shiftStart = LocalDateTime.of(2026, 6, 9, 5, 0);
    ...
    void offRouteTakesPriority() {
        ...
        var status = FleetPresenceAlertMonitor.classify(now.minusMinutes(1), offRoute, now, shiftStart, props);
    ...
    void offRouteTakesPriorityOverStaleGps() {
        Optional<OffRouteRecord> offRoute = Optional.of(new OffRouteRecord(Instant.now(), 300, 0, 0));
        var status = FleetPresenceAlertMonitor.classify(now.minusMinutes(40), offRoute, now, shiftStart, props);
        assertEquals(Optional.of(AssignedVehicleStatus.OFF_ROUTE), status);
    }

    @Test
    void freshWithinThresholdIsOk() {
        var status = FleetPresenceAlertMonitor.classify(now.minusMinutes(5), Optional.empty(), now, shiftStart, props);
        assertTrue(status.isEmpty());
    }

    @Test
    void seenThisShiftButStaleIsWentSilent() {
        var status = FleetPresenceAlertMonitor.classify(now.minusMinutes(40), Optional.empty(), now, shiftStart, props);
        assertEquals(Optional.of(AssignedVehicleStatus.WENT_SILENT), status);
    }

    @Test
    void neverSeenAndPastGraceIsNotStarted() {
        var status = FleetPresenceAlertMonitor.classify(null, Optional.empty(), now, shiftStart, props);
    ...
    void neverSeenWithinGraceIsNotReported() {
        ...
        var status = FleetPresenceAlertMonitor.classify(null, Optional.empty(), earlyShift, shiftStart, props);
        assertTrue(status.isEmpty());
    }
}
