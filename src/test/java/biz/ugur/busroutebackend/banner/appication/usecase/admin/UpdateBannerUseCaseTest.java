...
class UpdateBannerUseCaseTest {
    ...
    void shouldUpdateBannerSuccessfully() {
        ...
        StepVerifier.create(result)
                .assertNext(response -> {
                    assertEquals(BANNER_ID, response.getId());
                    assertEquals(TITLE, response.getTitle());
                    assertEquals(PROCESSED_IMAGE_URL, response.getImageUrl());
                    assertEquals(TARGET_URL, response.getTargetUrl());
                    assertTrue(response.getIsActive());
                    assertEquals(DISPLAY_ORDER, response.getDisplayOrder());
                    assertEquals(CONTENT, response.getContent());
                    assertEquals(START_DATE.atZone(ZoneId.systemDefault()).toInstant(), response.getStartDate());
                    assertEquals(END_DATE.atZone(ZoneId.systemDefault()).toInstant(), response.getEndDate());
                })
                .verifyComplete();
